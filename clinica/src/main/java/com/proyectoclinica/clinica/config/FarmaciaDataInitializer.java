        ...
        return args -> {
            if (repository.count() == 0) {
                ...
                List<Producto> productos = List.of(
                    Producto.builder().nombre("Paracetamol 500mg").descripcion("Caja x 100 tabletas. Analgésico y antipirético.").precio(new BigDecimal("15.50")).precioDescuento(new BigDecimal("12.50")).categoria("Medicamentos Generales").estado("Activo").imagenUrl("https://placehold.co/400x400/DCFCE7/166534?text=Medicina").stock(150).build(),
                    Producto.builder().nombre("Ibuprofeno 400mg").descripcion("Caja x 50 cápsulas blandas.").precio(new BigDecimal("22.00")).precioDescuento(null).categoria("Analgésicos").estado("Activo").imagenUrl("https://placehold.co/400x400/DBEAFE/1E40AF?text=Analgésico").stock(100).build(),
                    ...
                    Producto.builder().nombre("Jarabe para la tos con miel").descripcion("Frasco 120ml. Expectorante adulto.").precio(new BigDecimal("28.00")).precioDescuento(null).categoria("Respiratorio").estado("Activo").imagenUrl("https://placehold.co/400x400/FCE7F3/9D174D?text=Jarabe").stock(60).build(),
                    Producto.builder().nombre("Termómetro Digital").descripcion("Termómetro de uso clínico preciso con alarma auditiva.").precio(new BigDecimal("18.00")).precioDescuento(new BigDecimal("14.00")).categoria("Equipos Médicos").estado("Activo").imagenUrl("https://placehold.co/400x400/F3E8FF/6B21A8?text=Salud").stock(40).build()
                );
