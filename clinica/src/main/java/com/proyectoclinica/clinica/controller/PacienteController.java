...
public class PacienteController {
    ...
    public String dashboard(Model model, Principal principal) {
        ...
        if (paciente != null) {
            ...
            model.addAttribute("resultadosListosCount", resultadoLaboratorioRepository.countByPacienteIdAndEstado(paciente.getId(), "Disponible"));
            model.addAttribute("ultimosResultados", resultadoLaboratorioRepository.findByPacienteIdOrderByFechaExamenDesc(paciente.getId()).stream().limit(3).collect(Collectors.toList()));
        }
    ...
    public String procesarAgendarCita(@RequestParam Integer id_medico,
        ...
        if (paciente != null && medico != null) {
            ...
            if (id_cita != null) {
                // Validación de conflicto de horario para re-agendamiento (excluye la propia cita)
     ...
     * Integrante: 3
     */
    // EXPOSICIÓN RHOMEL: Aquí obtenemos los resultados de laboratorio listos asociados al paciente usando JPA.
    ...
    public String resultados(Model model, Principal principal) {
        ...
        model.addAttribute("pacienteGlobal", paciente);
        model.addAttribute("resultados", resultadoLaboratorioRepository.findByPacienteIdOrderByFechaExamenDesc(paciente.getId()));
        model.addAttribute("pageTitle", "Mis Resultados");
        model.addAttribute("view", "paciente/resultados");
        return LAYOUT;
    }

    // EXPOSICIÓN RHOMEL: Explicar cómo funciona el servicio RecetaService para separar la lógica de negocio del controlador.
    @GetMapping("/recetas")
    ...
    public String historial(Model model, Principal principal) {
        ...
        model.addAttribute("pacienteGlobal", paciente);
        model.addAttribute("atenciones", citaRepository.findByPacienteIdAndEstadoOrderByFechaCitaDesc(paciente.getId(), "Completada"));
    ...
    // EXPOSICIÓN RHOMEL: Aquí actualizamos los datos permitidos del paciente, excluyendo campos sensibles como el DNI.
    ...
    // EXPOSICIÓN RHOMEL: Endpoint añadido para cambiar la contraseña usando BCrypt, mejorando la seguridad.
    ...
    // EXPOSICIÓN RHOMEL: Lógica del carrito de compras, valida ítems y guarda la relación de Pedido y DetallePedido en cascada/loop.
    @PostMapping("/farmacia/comprar")
    @ResponseBody
    public ResponseEntity<?> procesarCompra(@RequestBody PedidoRequest pedidoReq, Principal principal) {
        Paciente paciente = getPacienteAutenticado(principal);
        if (paciente == null) return ResponseEntity.status(401).body(Map.of("success", false, "message", "No autenticado"));
        ...
        for (PedidoRequest.ItemPedido item : pedidoReq.getItems()) {
            ...
            if (producto != null) {
                BigDecimal precioFinal = producto.getPrecioDescuento() != null ? producto.getPrecioDescuento() : producto.getPrecio();
